...
import com.zbkj.service.mpay.bean.OrderState;
...
public class CallbackServiceImpl implements CallbackService {
    ...
    @Override
    public void mpay(MPayNotifyData mPayNotifyData) {
        if (!MPayUtil.verifySign(mPayNotifyData.getFieldMap(), MPayConfig.signKey)) {
            throw new CrmebException("签名不正确");
        }

        if (mPayNotifyData.getOrderState().equals(OrderState.SUCCESS)) {
            //支付成功
            StoreOrder orderParam = new StoreOrder();
            orderParam.setOutTradeNo(mPayNotifyData.getMerchantOrderNo());
//            orderParam.setUid(attachVo.getUserId());

            StoreOrder storeOrder = storeOrderService.getInfoByEntity(orderParam);
            if (ObjectUtil.isNull(storeOrder)) {
                throw new CrmebException("mpay error : 订单信息不存在==》" + mPayNotifyData.getMerchantOrderNo());
            }
            logger.error("");
            if (storeOrder.getPaid()) {
                throw new CrmebException("mpay error : 订单已处理==》" + mPayNotifyData.getMerchantOrderNo());
            }
